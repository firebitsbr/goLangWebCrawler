...
func cacheSites(c *cli.Context) {
	...
	if err := cacheData(db, data, bar); err != nil {
		log.Fatalln(err)
	}
}

func cacheData(db *bolt.DB, data *csv.Reader, bar *pb.ProgressBar) error {
...
func mustOpenDB(name string) *bolt.DB {
